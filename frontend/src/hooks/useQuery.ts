...
/**
 * Runs `fn` whenever `deps` change and tracks loading/success/error state.
 * `fn` is expected to be a fresh closure per render (it is only invoked,
 * never diffed), so it does not need to be memoized by the caller.
 */
